...
import (
	...
	omni "github.com/voxgig-sdk/weather-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WeatherError); is {
				out := map[string]any{
					"name":    "WeatherError",
...
func omniCtx(arg any, client *sdk.WeatherSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WeatherSDK); is {
			client = live
