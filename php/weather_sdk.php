...
class WeatherSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WeatherHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WeatherHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WeatherFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WeatherFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WeatherUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WeatherHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WeatherHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WeatherHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WeatherError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WeatherHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WeatherError("graphql_error",
                "WeatherSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
