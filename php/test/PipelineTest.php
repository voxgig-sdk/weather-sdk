...
{
    private static function utility(): WeatherUtility
    {
        return new WeatherUtility();
    }
    ...
    private static function ctx(array $over = []): WeatherContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new WeatherContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new WeatherOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof WeatherError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new WeatherOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = WeatherMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new WeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = WeatherMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new WeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = WeatherMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = WeatherMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = WeatherMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new WeatherSpec([]),
            'response' => null,
            'result' => new WeatherResult([]),
        ]);
        [, $err] = WeatherMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new WeatherSpec(['step' => 's']),
            'response' => new WeatherResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new WeatherResult([]),
        ]);
        [, $err] = WeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherSpec(['step' => 's']),
            'response' => new WeatherResponse(self::resp(200, ['v' => 1])),
            'result' => new WeatherResult([]),
        ]);
        [, $err] = WeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new WeatherSpec(['step' => 's']),
            'response' => new WeatherResponse(self::resp(200, ['v' => 2])),
            'result' => new WeatherResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        WeatherMakeResponse::call($ctx);
    ...
    {
        $preset = new WeatherResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new WeatherSpec([]),
            'response' => new WeatherResponse([]),
            'result' => new WeatherResult([]),
        ...
        [$got, $err] = WeatherMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new WeatherResult([])]);
        [, $err] = WeatherMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new WeatherSpec([]), 'result' => null]);
        [, $err] = WeatherMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherSpec(['step' => 's']),
            'result' => new WeatherResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new WeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new WeatherSpec(['step' => 's']),
            'result' => new WeatherResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new WeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = WeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new WeatherResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new WeatherSpec([]),
            'result' => new WeatherResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = WeatherMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new WeatherError('boom', 'boom');
        $utility->fetcher = function (WeatherContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = WeatherMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (WeatherContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (WeatherContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new WeatherSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = WeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new WeatherResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new WeatherSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = WeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = WeatherMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = WeatherMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new WeatherResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, WeatherDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new WeatherResult(['ok' => false])]);
        $this->expectException(WeatherError::class);
        WeatherDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new WeatherResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', WeatherMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        WeatherMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new WeatherBaseFeature();
        $b = new WeatherBaseFeature();
        WeatherFeatureAdd::call($ctx, $a);
        WeatherFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): WeatherBaseFeature
    {
        $f = new WeatherBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        WeatherFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): WeatherContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new WeatherSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = WeatherPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new WeatherResponse(['headers' => null]),
            'result' => new WeatherResult([]),
        ]);
        WeatherResultHeaders::call($ctx);
