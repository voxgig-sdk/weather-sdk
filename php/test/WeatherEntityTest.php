...
class WeatherEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set WEATHER_TEST_WEATHER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function weather_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("WEATHER_TEST_WEATHER_ENTID");
    ...
    if ($env["WEATHER_TEST_LIVE"] === "TRUE") {
        ...
        $client = new WeatherSDK(Helpers::to_map($merged_opts));
    }
