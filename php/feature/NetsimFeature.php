...
class WeatherNetsimFeature extends WeatherBaseFeature
{
    ...
    private function _simulate(WeatherContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(WeatherContext $ctx, array $applied): void
    {
        $client = $this->client;
