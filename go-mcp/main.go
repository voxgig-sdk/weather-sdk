// Command Weather-mcp is an MCP server exposing the Weather
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Weather_list  — list records of an entity
//	Weather_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: WEATHER_APIKEY carries the API key and
	// WEATHER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEATHER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEATHER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWeatherSDK(opts)
