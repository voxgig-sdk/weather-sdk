...
[MCP](https://modelcontextprotocol.io) server exposing the Weather SDK as
two agent tools — `weather_list` and `weather_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-mcp)
make build

# 2. Provide credentials via the environment
export WEATHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user weather \
  -- /absolute/path/to/weather-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./weather-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// weather_list: first page of records
{ "entity": "weather" }
{ "entity": "weather", "query": { } }

// weather_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user weather \
     -- "$PWD"/dist/*/weather-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `weather_list` and `weather_load` tools now appear
   in new sessions. Ask the agent to *"list weather using weather"*
   and it calls `weather_list` with `{"entity":"weather"}`.
...
```sh
export WEATHER_APIKEY=sk_live_xxx            # API key
export WEATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `weather_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `weather_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEATHER_APIKEY` | API key sent with every request. |
| `WEATHER_BASE` | Optional override of the API base URL. |
...
```sh
./weather-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather_load","arguments":{"entity":"weather","query":{"id":1}}}}'
```
