...
MCP server exposing the Weather SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `weather_list` | `entity`, optional `query` map | First page of records as JSON |
| `weather_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `weather_list` and
`weather_load` tools become available in new sessions.
