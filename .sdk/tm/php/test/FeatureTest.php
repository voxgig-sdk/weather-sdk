...
class FtCtrl extends WeatherControl
{
...
class FtClient
{
    ...
    public function __construct(WeatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): WeatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new WeatherUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (WeatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeatherConfig::make_feature($name);
    ...
    private static function buildUrl(WeatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeatherResponse($fetched);
        }
    ...
    private function fail(WeatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
