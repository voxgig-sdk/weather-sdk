# Weather SDK utility: make_request
...
module WeatherUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = WeatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeatherResponse.new(fetched)
    else
