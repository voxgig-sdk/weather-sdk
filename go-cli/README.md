...
boru-driven command-line client **and** interactive REPL for the Weather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-cli)
make build
...
./weather-cli load 1 weather            # {id:1} shorthand
./weather-cli load '{id:1}' weather       # explicit match map

# 5. Override the API base URL for a single call
WEATHER_BASE=https://api.example.com ./weather-cli load 1 weather

# 6. No arguments -> interactive REPL
./weather-cli
weather> load 1 weather
weather> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/weather-cli load 1 weather
   ```
...
```sh
./weather-cli load 1 weather          # scalar shorthand for {id:1}
./weather-cli load '{id:1}' weather     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `weather>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/weather-cli`. |
...
(`./weather-cli <expr>`) and interactively (the REPL), and expressions compose the
