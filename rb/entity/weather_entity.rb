...
class WeatherEntity
  ...
  def data_set(args)
    if args
      @_data = WeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Weather, Hash] the loaded Weather; raises WeatherError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WeatherHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
