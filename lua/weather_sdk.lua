...
WeatherSDK._make_feature = _make_feature


function WeatherSDK.new(options)
  local self = setmetatable({}, WeatherSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function WeatherSDK:options_map()
...
function WeatherSDK:get_utility()
  return Utility.copy(self._utility)
end


function WeatherSDK:get_root_ctx()
  return self._rootctx
end


function WeatherSDK:prepare(fetchargs)
...
function WeatherSDK:direct(fetchargs)
...
-- by passing it.
function WeatherSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function WeatherSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "WeatherSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function WeatherSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = WeatherSDK.new(sdkopts)
  sdk.mode = "test"
