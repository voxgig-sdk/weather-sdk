...
//     (\`new WeatherSDK(...)\` / \`WeatherSDK.test(...)\` become
//     \`WeatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new WeatherSDK(...)\` and any
// \`WeatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WeatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WeatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
